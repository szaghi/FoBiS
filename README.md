...
| 🤖 JSON output | | | |
| Pass `--json` to `fobis build`, `fobis clean`, or `fobis fetch` to get a single machine-readable JSON object instead of human-readable output. Ideal for CI scripts and AI agent workflows. | | | |
