...
| Pass `--json` to `fobis build`, `fobis clean`, or `fobis fetch` to get a single machine-readable JSON object instead of human-readable output. Ideal for CI scripts and AI agent workflows. | Install the bundled `/fobis` Claude Code skill for expert AI assistance — fobos authoring, error diagnosis, dependency setup, and more, right in your editor. | | |
