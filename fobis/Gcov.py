...
__str_procedure__ = (
    r"^(\s*)"  # initial white spaces
    + r"("
    + __str_kw_elemental__
    + r"|"
    + __str_kw_pure__
    + r")?\s+"  # "elemental" or "pure" attribute
    + r"(?P<ptype>"
    + __str_kw_function__
    + r"|"
    + __str_kw_subroutine__
    + r")\s+"  # type of procedure
    + r"(?P<pname>"
    + __str_name__
    + r"\s*).*"
)  # name of procedure
...
class Gcov:
    ...
    def _metrics(self):
        ...
        if len(self.coverage) > 0:
            ...
            if lnumber > 0:
                elnumber_per = int(round(elnumber * 100.0 / lnumber))
                unelnumber_per = int(round(unelnumber * 100.0 / lnumber))
            else:
                elnumber_per = 0
                unelnumber_per = 0
            if elnumber > 0:
        ...
        if len(self.procedures) > 0:
            ...
            if pnumber > 0:
                epnumber_per = int(round(epnumber * 100.0 / pnumber))
                unepnumber_per = int(round(unepnumber * 100.0 / pnumber))
            else:
                epnumber_per = 0
                unepnumber_per = 0
            if epnumber > 0:
    ...
    def parse(self):
        ...
        def _coverage_update(ignoring, cov_num, line_num, text, coverage):
            ...
            if re.search(r"\bLCOV_EXCL_START\b", text):
                if ignoring:
                    sys.stderr.write("Warning: %s:%d: nested LCOV_EXCL_START, please fix\n" % (self.filename, line_num))
                ignoring = True
            elif re.search(r"\bLCOV_EXCL_END\b", text):
                if not ignoring:
                    sys.stderr.write(
                        "Warning: %s:%d: LCOV_EXCL_END outside of "
            ...
            if cov_num == "-":
                ...
                if (
                    ignoring
                    or text.lstrip().startswith(("inline", "static"))
                    or text.strip() == "}"
                    or re.search(r"\bLCOV_EXCL_LINE\b", text)
                ):
                    coverage.append(None)
                else:
                    coverage.append(0)
            ...
            if os.path.exists(self.filename):
                with open(self.filename) as source:
                    for line in source:
                        ...
                        ignoring = _coverage_update(
                            ignoring=ignoring, cov_num=cov_num, line_num=line_num, text=text, coverage=self.coverage
                        )
        self._metrics()
    ...
    def save(self, report_format="markdown", output=None, graphs=False):
        ...
        if len(self.metrics) > 0:
            if report_format == "markdown":
                string.append(
                    "### Coverage analysis of *" + os.path.splitext(os.path.basename(self.filename))[0] + "*\n"
                )
                string.append("\n")
                if self.metrics["coverage"]:
                    string.append("|Lines| | |\n")
                    ...
                    string.append(
                        "|Executed lines              |"
                        + self.metrics["coverage"][1]
                        + "|"
                        + self.metrics["coverage"][3]
                        + "%|\n"
                    )
                    string.append(
                        "|Unexecuted lines            |"
                        + self.metrics["coverage"][2]
                        + "|"
                        + self.metrics["coverage"][4]
                        + "%|\n"
                    )
                    ...
                    string.append(
                        _mermaid_pie(
                            "Lines (" + self.metrics["coverage"][3] + "% covered)",
                            self.metrics["coverage"][1],
                            self.metrics["coverage"][2],
                        )
                    )
                if self.metrics["procedures"]:
                    ...
                    string.append(
                        ...
                        + "|"
                        + self.metrics["procedures"][3]
                        + "%|\n"
                    )
                    string.append(
                        "|Unexecuted procedures       |"
                        + self.metrics["procedures"][2]
                        + "|"
                        + self.metrics["procedures"][4]
                        + "%|\n"
                    )
                    ...
                    string.append(
                        _mermaid_pie(
                            "Procedures (" + self.metrics["procedures"][3] + "% covered)",
                            self.metrics["procedures"][1],
                            self.metrics["procedures"][2],
                        )
                    )
                ...
                if any(proc[2] > 0 for proc in self.procedures):
                    for proc in sorted(self.procedures, key=itemgetter(2), reverse=True):
                        if proc[2] > 0:
                            string.append(
                                " + *" + proc[0] + "* **" + proc[1] + "**: tested **" + str(proc[2]) + "** times\n"
                            )
                else:
                    string.append(" + *none*\n")
