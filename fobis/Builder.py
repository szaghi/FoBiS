...
class Builder:
    ...
    def __init__(
        ...
        self._sanitize_dirs(
            src_dir=cliargs.src,
            build_dir=cliargs.build_dir,
            obj_dir=cliargs.obj_dir,
            mod_dir=cliargs.mod_dir,
            lib_dir=cliargs.lib_dir,
            dinc=cliargs.include,
        )
        self._sanitize_files(
            libs=cliargs.libs, vlibs=cliargs.vlibs, ext_libs=cliargs.ext_libs, ext_vlibs=cliargs.ext_vlibs
        )
        self._set_preprocessor(
            preprocessor=cliargs.preprocessor,
    ...
    def _set_preprocessor(self, preprocessor, preprocessor_dir, preprocessor_ext, preprocessor_no_o, preprocessor_args):
        """
        ...
        if self.preprocessor:
            ...
            if not preprocessor_exist:
                self.print_w(
                    "Error: " + self.preprocessor + " is not in your PATH! You cannot use -preprocessor switch."
                )
        return
    ...
    def _link_command(self, file_to_build, output=None, nomodlibs=None, submodules=None, mklib=None):
        ...
        link_cmd = (
            self.cmd_link
            + " "
            + self._get_libs_link_command(
                file_to_build=file_to_build, nomodlibs=nomodlibs, submodules=submodules, mklib=mklib
            )
            + " -o "
            + exe
        )
    ...
    def _get_libs_link_command(
        ...
        link_cmd = (
            "".join([os.path.join(self.obj_dir, s + " ") for s in objs])
            + "".join([s + " " for s in self.libs])
            + "".join([s + " " for s in self.vlibs])
        )
    ...
    def _get_hierarchy(self, file_to_build):
        ...
        order_max = (
            max(
                [p for p in file_to_build.pfile_dep_all if not p.include and p.to_compile],
            ...
            if results[0][0] != 0:
                ...
                if not quiet:
                    self.print_w(
                        ...
                        + file_to_build.name
                        + " fails! Removing from the list of non-module libraries linked into the target!"
                    )
        return build_ok
    ...
    def build(
        ...
        if file_to_build.program:
            link_cmd, exe = self._link_command(
        ...
        if track:
            ...
            with open(track_file_name, "w") as track_file:
                ...
                track_file.writelines(
                    "output = " + self.get_output_name(file_to_build=file_to_build, output=output) + "\n"
                )
                if file_to_build.program:
                    ...
                    track_file.writelines(
                        "mod_file = " + os.path.join(self.mod_dir, file_to_build.basename + ".mod") + "\n"
                    )
        return build_ok
    ...
    def verbose(self, quiet: bool = False) -> str:
        ...
        if not quiet:
            ...
            if len(self.vlibs) > 0:
                message += (
                    "  Linked volatile libraries with full path: " + "".join([s + " " for s in self.vlibs]) + "\n"
                )
            if len(self.ext_libs) > 0:
