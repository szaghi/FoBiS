...
class FoBiSConfig(object):
    ...
    for root, _, files in os.walk('.'):
      for filename in files:
        if os.path.splitext(os.path.basename(filename))[1] == '.gcov':
          ...
          gcov = Gcov(filename=os.path.join(root, filename))
          gcov.parse()
          gcov.save(output=os.path.join(self.cliargs.gcov_analyzer, os.path.basename(filename) + '-report.md'))
    return
  ...
  def run_fobis(self, fake_args=None):
    ...
    if self.cliargs.which == 'rule':
      ...
      elif self.cliargs.coverage:
        result = syswork(self._coverage())
        if result[0] != 0:
          self._print_r(result[1])
        else:
          self._print_b(result[1])
