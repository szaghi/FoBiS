...
class FoBiSConfig(object):
  ...
  def _get_parsed_files_list(self, src_dir=None, is_doctest=False):
    """
    ...
    ----------
    src_dir: str
      root directory into which search; if omitted use self.cliargs.src
  ...
  def _build(self):
    ...
    for pfile in pfiles:
      if pfile.nomodlib:
        build_ok = builder.build(file_to_build=pfile, verbose=self.cliargs.verbose, log=self.cliargs.log)
        if build_ok:
  ...
  def _doctests(self):
    """
    Method for running FoBiS.py in doctests mode.
    """
    builder = Builder(cliargs=self.cliargs, print_n=self._print_b, print_w=self._print_r)
    pfiles = self._get_parsed_files_list()
    # building dependencies hierarchy
    ...
    for pfile in pfiles:
      if pfile.nomodlib:
        ...
        if build_ok:
          nomodlibs.append(pfile.basename + ".o")
    # creating volatile doctests programs
    doctests = []
    doctests_dirs = []
    for pfile in pfiles:
      if pfile.doctest:
        if pfile.doctest.to_test:
          ...
          doctests += self._get_parsed_files_list(src_dir=doc_dir, is_doctest=True)
    # appending other parsed files to the doctests list
    for pfile in pfiles:
      doctests.append(pfile)
    dependency_hiearchy(builder=builder, pfiles=doctests, print_w=self._print_r, force_compile=self.cliargs.force_compile)
    # building doctests
    for test in doctests:
      if test.is_doctest:
        ...
        self._print_b('executing doctest ' + os.path.basename(test_exe))
        ...
        if result[0] == 0:
          ...
          if result[1].strip() == expected_result:
            self._print_b('doctest passed')
          else:
            self._print_r('doctest failed!')
            self._print_b('  result obtained: "' + result[1].strip() + '"')
            self._print_b('  result expected: "' + expected_result + '"')
        if not self.cliargs.keep_volatile_doctests:
          os.remove(test_exe)
    if not self.cliargs.keep_volatile_doctests:
  ...
  def run_fobis(self, fake_args=None):
    ...
    if self.cliargs.which == 'rule':
      ...
      if self.cliargs.which == 'build':
        self._build()
      if self.cliargs.which == 'doctests':
        self._doctests()
    return
