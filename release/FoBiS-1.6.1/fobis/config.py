...
class FoBiSConfig(object):
  ...
  def _save_makefile(self, pfiles, builder):
    ...
    string.append("\n")
    string.append("#main building variables\n")
    string.append("DSRC    = " + self.cliargs.src + "\n")
    string.append("DOBJ    = " + os.path.normpath(builder.obj_dir) + "/\n")
    string.append("DMOD    = " + os.path.normpath(builder.mod_dir) + "/\n")
    string.append("VPATH   = $(DSRC) $(DOBJ) $(DMOD)" + "\n")
    string.append("DEXE    = " + os.path.normpath(builder.build_dir) + "/\n")
    string.append("MKDIRS  = $(DOBJ) $(DMOD) $(DEXE)" + "\n")
    string.append("LIBS    =" + "".join(" " + l for l in builder.libs) + "\n")
    string.append("FC      = " + builder.compiler.fcs + "\n")
    string.append("OPTSC   = " + builder.compiler.cflags + " " + builder.compiler.modsw + builder.mod_dir + "\n")
    string.append("OPTSL   = " + builder.compiler.lflags + " " + builder.compiler.modsw + builder.mod_dir + "\n")
    ...
    with open(self.cliargs.makefile, "w") as mk_file:
      mk_file.writelines("".join(string))
    # compiling rules
    for pfile in pfiles:
      pfile.save_makefile(makefile=self.cliargs.makefile, builder=builder)
    # phony auxiliary rules
    string = []
    string.append("#phony rules \n")
    ...
    with open(self.cliargs.makefile, "a") as mk_file:
